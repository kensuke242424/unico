...
本アプリの今後の課題として、アーキテクチャに沿った適切な設計が出来ていない点がある。  
MVVMアーキテクチャを採用しているが、自身のアーキテクチャへの理解の浅いまま実装をしてきてしまったことが原因だ。  
「とりあえずアプリとして動く」が、各レイヤーごとの責務の分離が出来ておらず、今後の運用や保守という視点で見たときに  
カオスなソースコードになっている部分が多い。  

今後の機能追加や変更に柔軟に対応していくためにも、まずは設計を見直し、以下の点でリファクタリングを実施していく。

**1. Firebase通信ロジックをViewModelから分離（Firestore, Storage）**  

**2. 一つのViewに一つのViewModel構成**  

**3. Viewの肥大化改善、UIとロジックの分離**  
