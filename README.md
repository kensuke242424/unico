...
**1. 馴染みがないUIにおけるUXとのバランス**

今回のように一般的なアプリのUIとは少し異なるレイアウトを実装した時、  
ユーザーにとっては、普段慣れている感覚とは違う操作方法やパーツの配置はストレスになり、  
アプリから離脱してしまう原因となる可能性に十分気をつけなければならない。  
「次どう操作すればいいのかわからない」と感じそうなポイントを探って、  
操作チュートリアルを挿入するなど、対策していく必要がある。
...
各メソッドやAPIが内部で行なっている処理にもっと目を向けることが必要だと感じた。

また、開発の後半になればなるほど、  
状態変数を含めたプロパティの数が増えていくことで管理が難しくなっていくことに悩んだ。  
思考コストが少ない、管理がしやすいプログラムの書き方の引き出しが必要。

<br>

**3. ファイルの肥大化をどう整理していくか** 

多数の機能やインタラクションが存在するファイルにおいて、各コンポーネントをどのように切り分け、  
どうすれば保守しやすい状態にできるかという点で、頭を抱えることが多かった。  
一時期、積極的に各コンポーネントを切り分けて単一のViewとして管理しようとしたが、  
引数の定義が冗長になるなど、むしろ管理がしづらくなってしまうという経験もした。  
他の方のソースコードを拝見してみると、`@ViewBuilder func`で切り分けていたり、  
`Extension`で追加していっていたりと、人によって書き方に違いがあった。  
この課題は奥が深いのかもしれない。その時々で、ベターなコーディングを探る必要がある。
